...
public class GameState extends State{
	...
	public void divideMeteor(Meteor meteor) {
		...
		for (int i = 0; i < size.quantity; i++) {
			movingObjects.add(new Meteor(meteor.getPosition(), new Vector2D(0, 1).setDirection(Math.random()*Math.PI*2), 
					Constants.METEOR_INIT_VEL*Math.random() + 1, textures[(int)(Math.random()*textures.length)], this, newSize));
		}
	}
	...
	private void startWave() {
		...
		for(int i = 0; i < meteors; i++) {
			...
			movingObjects.add(new Meteor(new Vector2D(x, y), new Vector2D(0, 1).setDirection(Math.random()*Math.PI*2), 
					Constants.METEOR_INIT_VEL*Math.random() + 1, texture, this, Size.BIG));
	...
	private void spawnUfo() {
		...
		movingObjects.add(new Enemy(new Vector2D(x, y), new Vector2D(), Constants.UFO_MAX_VEL, Assets.ufo, path, this));
	...
	public void gameOver() {
		Message gameOverMsg = new Message(PLAYER_START_POSITION, true, "GAME OVER", Color.WHITE, true, Assets.fontBig);
