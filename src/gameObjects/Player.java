...
public class Player extends MovingObject{
	...
	public void update(float dt) 
	{
		...
		if (Keys.SHOOT &&  fireRate > fireSpeed && !spawning){
			...
			if (doubleGunOn) {
				...
				Shot l = new Shot(leftGun, heading, Constants.LASER_VEL, angle, Assets.blueLaser, gameState);
				Shot r = new Shot(rightGun, heading, Constants.LASER_VEL, angle, Assets.blueLaser, gameState);
				
				gameState.getMovingObjects().add(0, l);
				gameState.getMovingObjects().add(0, r);
				
			} else {
				gameState.getMovingObjects().add(0,new Shot(
	...
	public void draw(Graphics g) {
		...
		AffineTransform at1 = AffineTransform.getTranslateInstance(position.getX() + width/2 - 6,
				position.getY() + height/2 + 26);
		
		at1.rotate(angle, 6, -26);
